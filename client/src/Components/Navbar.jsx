    ...
    return (
        <>
            <nav className="flex items-center justify-between flex-wrap bg-blue-500 p-6">
                ...
                {page === "auth" && <DarkModeSwitch className="mx-auto sm:inline-block sm:mr-4" id="theme-switch" onChange={toggleDarkMode} checked={isDarkMode === 'false' ? true : false} size={30} />}
                {page !== "auth" &&
                    <>
                        ...
                        <div className={`w-full block flex-grow justify-end lg:flex lg:items-center lg:w-auto ${isMenuOpen ? 'block' : 'hidden'}`}>
                            <DarkModeSwitch className="mr-4 mx-2 mt-4 mb-2 sm:inline-block" id="theme-switch" onChange={toggleDarkMode} checked={isDarkMode === 'false' ? true : false} size={30} />
                            {page === "candidate" && <Link to="/home" className="sm:inline-block text-sm px-2 py-1.5 mr-4 leading-none border rounded text-white border-white hover:border-transparent hover:text-blue-500 hover:bg-white mt-4 lg:mt-0 dark:text-gray-900 dark:border-gray-900 dark:hover:text-white dark:hover:bg-gray-700 sm:text-base sm:px-4 py-2">
                                Home
                            </Link>}
                            {page === "" && <Link to="/add_candidate" className="sm:inline-block text-sm px-2 py-1.5 mr-4 leading-none border rounded text-white border-white hover:border-transparent hover:text-blue-500 hover:bg-white mt-4 lg:mt-0 dark:text-gray-900 dark:border-gray-900 dark:hover:text-white dark:hover:bg-gray-700 sm:text-base sm:px-4 py-2">Add a Candidate</Link>}
                            {page !== "auth" && <div onClick={handleLogout} className="hover:cursor-pointer inline-block text-sm px-2 py-1.5 mr-4 leading-none border rounded text-white border-white hover:border-transparent hover:text-blue-500 hover:bg-white mt-4 lg:mt-0 dark:text-gray-900 dark:border-gray-900 dark:hover:text-white dark:hover:bg-gray-700 sm:text-base">Log Out</div>}
                        </div>
                    </>
                }
            </nav>
